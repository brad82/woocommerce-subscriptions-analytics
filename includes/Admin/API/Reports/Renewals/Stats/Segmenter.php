...
class Segmenter extends ReportsSegmenter {
	 ...
	 *
	 * @param string $unique_Subscriptions_table Name of SQL table containing the order-level segmenting info.
	 *
	 * @return array Column => SELECT query mapping.
	 */
	protected function get_segment_selections_order_level( $unique_Subscriptions_table ) {
		$columns_mapping = array(
			'Subscriptions_count' => "COUNT($unique_Subscriptions_table.order_id) AS Subscriptions_count",
			'avg_items_per_order' => "AVG($unique_Subscriptions_table.num_items_sold) AS avg_items_per_order",
			'avg_order_value'     => "SUM($unique_Subscriptions_table.net_total) / COUNT($unique_Subscriptions_table.order_id) AS avg_order_value",
			'total_customers'     => "COUNT( DISTINCT( $unique_Subscriptions_table.customer_id ) ) AS total_customers",
		);
	 ...
	 * @param string $table_name Name of SQL table which is the stats table for Subscriptions.
	 * @param array  $totals_query Array of SQL clauses for totals query.
	 * @param string $unique_Subscriptions_table Name of temporary SQL table that holds unique Subscriptions.
	 *
	 * @return array
	 */
	protected function get_product_related_totals_segments( $segmenting_selections, $segmenting_from, $segmenting_where, $segmenting_groupby, $segmenting_dimension_name, $table_name, $totals_query, $unique_Subscriptions_table ) {
		global $wpdb;

		$product_segmenting_table = $wpdb->prefix . 'wc_order_product_lookup';

		// Can't get all the numbers from one query, so split it into one query for product-level numbers and one for order-level numbers (which first need to have Subscriptions uniqued).
		...
		// Order level numbers.
		// As there can be 2 same product ids (or variation ids) per one order, the Subscriptions first need to be uniqued before calculating averages, customer counts, etc.
		$segments_Subscriptions = $wpdb->get_results(
			"SELECT
				    $unique_Subscriptions_table.$segmenting_dimension_name AS $segmenting_dimension_name
				...
				) AS $unique_Subscriptions_table
				GROUP BY
				    $unique_Subscriptions_table.$segmenting_dimension_name",
			ARRAY_A
		); // phpcs:ignore cache ok, DB call ok, unprepared SQL ok.

		$totals_segments = $this->merge_segment_totals_results( $segmenting_dimension_name, $segments_products, $segments_Subscriptions );
	 ...
	 * @return array
	 */
	protected function get_product_related_intervals_segments( $segmenting_selections, $segmenting_from, $segmenting_where, $segmenting_groupby, $segmenting_dimension_name, $table_name, $intervals_query, $unique_Subscriptions_table ) {
		global $wpdb;
		...
		$segments_Subscriptions = $wpdb->get_results(
			"SELECT
					$unique_Subscriptions_table.time_interval AS time_interval,
				    $unique_Subscriptions_table.$segmenting_dimension_name AS $segmenting_dimension_name
				    {$segmenting_selections['order_level']}
				FROM
				(
				...
				GROUP BY
				    time_interval, $unique_Subscriptions_table.$segmenting_dimension_name
				$segmenting_limit",
			ARRAY_A
		); // phpcs:ignore cache ok, DB call ok, unprepared SQL ok.

		$intervals_segments = $this->merge_segment_intervals_results( $segmenting_dimension_name, $segments_products, $segments_Subscriptions );
	...
	protected function get_segments( $type, $query_params, $table_name ) {
		...
		if ( 'product' === $this->query_args['segmentby'] ) {
			...
			$order_level_columns       = $this->get_segment_selections_order_level( $unique_Subscriptions_table );
			...
			$segments = $this->get_product_related_segments( $type, $segmenting_selections, $segmenting_from, $segmenting_where, $segmenting_groupby, $segmenting_dimension_name, $table_name, $query_params, $unique_Subscriptions_table );
			...
			$coupon_level_columns  = $this->segment_selections_Subscriptions( $table_name, $coupon_override );
			...
			$customer_level_columns = $this->segment_selections_Subscriptions( $table_name );
